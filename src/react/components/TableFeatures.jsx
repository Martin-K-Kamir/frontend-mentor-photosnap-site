...
export default function TableFeatures() {
    return (
        <table className="[ table-features ] [ stack ]" data-stack-first="true" data-inherit-space='true'>
            <thead>
            <tr>{data.tableFeatures.headerCols.map((headerCol, index) => <th key={`key${index}`} scope="col" className={`[ title-1 ] [ text-uppercase ${index !== 0 ? "hide//below-md" : ""} ]`}>{headerCol}</th>)}</tr>
            </thead>
            ...
            {data.tableFeatures.headerRows.map((headerRow, index) => <tr key={`key${index}`} className="[ table-features__group ] [ flow ]">
                ...
                {data.tableFeatures.plans.map((plan, index) => <td key={`key${index}`}>
                    <span>
